...
// 1. Catch the userProfile prop here
const RestaurantDashboard = ({ userProfile }) => {
  ...
  const markAsReady = async (orderId) => {
    ...
    const { error } = await supabase.from('orders').update({ status: 'ready', pickup_pin: generatedPin }).eq('id', orderId);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      {/* Internal Tabs Nav */}
      <nav className="bg-white border-b px-6 py-4 sticky top-[73px] z-20 flex justify-between items-center shadow-sm">
        <h1 className="text-xl font-black text-[#003366] tracking-tighter uppercase">Kitchen</h1>
        <div className="flex bg-gray-100 p-1 rounded-xl">
          <button 
            onClick={() => setActiveTab('orders')} 
            className={`px-5 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'orders' ? 'bg-white shadow-sm text-[#003366]' : 'text-gray-400'}`}
          >
            Orders
          </button>
          <button 
            onClick={() => setActiveTab('menu')} 
            className={`px-5 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'menu' ? 'bg-white shadow-sm text-[#003366]' : 'text-gray-400'}`}
          >
            Menu
          </button>
        </div>
      ...
      <main className="max-w-4xl mx-auto p-6">
        {activeTab === 'orders' ? (
          <div className="space-y-4">
            {orders.length === 0 ? (
              <p className="text-center text-gray-400 py-10 font-medium">No incoming orders yet.</p>
            ) : (
              orders.map((order) => (
                <div key={order.id} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex justify-between items-center">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[10px] font-black bg-blue-50 text-[#003366] px-2 py-0.5 rounded uppercase">
                        ORD-{order.id.toString().slice(-4)}
                      </span>
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase ${order.status === 'ready' ? 'bg-green-50 text-green-600' : 'bg-orange-50 text-orange-600'}`}>
                        {order.status}
                      </span>
                    </div>
                  ...
                  {order.status === 'ready' ? (
                    ...
                    <button 
                      ...
                      className="bg-[#003366] text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-900 transition-all active:scale-95"
                    >
                      Mark Ready
                    </button>
                  )}
                ...
                <div key={item.id} className="p-5 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="relative group w-20 h-20">
                      <img src={item.image_url} className="w-full h-full rounded-2xl object-cover border border-gray-100" alt={item.name} />
                  ...
                  <div className="text-right">
                    {editingId === item.id ? (
                      <div className="flex items-center gap-2">
                        <input 
                          type="number" 
                          className="w-20 p-2 border rounded-lg text-sm font-bold outline-none border-[#E77206]" 
                          value={newPrice} 
                      ...
                      <div className="flex flex-col items-end">
                        ...
                        <button 
                          onClick={() => { setEditingId(item.id); setNewPrice(item.price); }} 
                          className="text-[#E77206] text-[10px] font-black uppercase tracking-tighter"
                        >
                          Update Price
                        </button>
                      </div>
